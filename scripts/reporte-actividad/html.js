// Render del HTML del reporte. Página autocontenida: CSS y JS inline, sin
// assets externos, sin red — se abre con doble click desde scripts/reportes/.
//
// El índice (tiles, gráfico, tabla) se renderiza acá en Node, así el reporte
// sigue siendo legible sin JS. El detalle por usuario lo arma el cliente a
// partir del payload embebido (ver cliente.js).

import { CLIENTE_JS } from './cliente.js'
...
// El payload va dentro de un <script type="application/json">, donde el único
// carácter peligroso es `<` (un "</script>" en el nombre de un ejercicio
// cerraría el bloque). < es JSON válido, así que JSON.parse lo revierte.
function embeberJson(payload) {
...
const CSS = `
  ...
  h1 { font-size: 1.3rem; }
  h2 { font-size: 1.2rem; margin: 0.5rem 0 0.2rem; }
  h3 { font-size: 1rem; margin: 2rem 0 0.75rem; }
  h4 { font-size: 0.95rem; margin: 0 0 0.5rem; }
  h5 { font-size: 0.8rem; text-transform: uppercase; letter-spacing: 0.03em; color: #555; margin: 1rem 0 0.4rem; }
  ...
  tr[data-uid]:hover { background: #eef3ff; }
  tr[data-uid]:focus { outline: 2px solid #4f7cff; outline-offset: -2px; }
  tr[data-uid] td:first-child::after { content: ' ›'; color: #4f7cff; font-weight: 700; }
  .hint { color: #666; font-size: 0.8rem; margin: 0 0 0.5rem; }
  .volver { display: inline-block; margin-bottom: 1rem; color: #4f7cff; text-decoration: none; font-weight: 600; }
  .volver:hover { text-decoration: underline; }
  .card { background: #fff; border: 1px solid #e2e2e2; border-radius: 8px; padding: 1rem 1.25rem; margin-bottom: 1rem; }
  .card table { border: none; border-radius: 0; margin-bottom: 0.5rem; }
  .vacio { color: #888; font-style: italic; }
  .chip { display: inline-block; margin-left: 0.5rem; padding: 0.1rem 0.45rem; border-radius: 999px; background: #fff6e5; color: #b45309; font-size: 0.7rem; font-weight: 600; vertical-align: middle; }
  .sesion { background: #fff; border: 1px solid #e2e2e2; border-radius: 8px; padding: 0.6rem 1rem; margin-bottom: 0.5rem; }
  .sesion summary { cursor: pointer; display: flex; flex-wrap: wrap; gap: 0.75rem; align-items: baseline; }
  .sesion summary::-webkit-details-marker { color: #aaa; }
  .sesion .fecha { font-weight: 700; font-variant-numeric: tabular-nums; }
  .sesion .ruta { color: #333; }
  .sesion .meta { color: #666; font-size: 0.8rem; margin-left: auto; }
  .sesion table { margin-top: 0.75rem; border: none; }
  .sesion .nota { margin: 0.6rem 0 0; color: #555; font-style: italic; }
`
...
export function renderizarHTML(payload) {
  ...
  const filas = usuarios.map(u => `
    <tr class="${u.sesionesVsDias ? 'fila-alerta' : ''}" data-uid="${escapeHtml(u.uid)}" tabindex="0">
...
<body>
  <div id="vista-indice">
    ...
    <p class="hint">Clickeá una fila para ver el detalle del usuario.</p>
    <table>
  ...
  <div id="vista-detalle" hidden></div>

  <script type="application/json" id="datos">${embeberJson(payload)}</script>
  <script>${CLIENTE_JS}</script>
</body>
</html>`
}
