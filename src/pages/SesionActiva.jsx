...
export default function SesionActiva() {
  ...
  useEffect(() => {
    if (!usuario) return
    getSesionesConResumen(usuario.id).then(setSesionesCache).catch(e => console.error(e))
  }, [usuario])
  ...
  async function completarSerie() {
    ...
    } catch (e) { console.error(e); setGuardando(false); return }
  ...
  if (ejercicios.length === 0) return (
    <div style={s.page}>
      <div style={s.body}>
        <p style={s.muted}>Este día no tiene ejercicios.</p>
        <button style={s.completarBtn} onClick={() => navigate(-1)}>Volver</button>
      </div>
    </div>
  )
  ...
  return (
    ...
    >
      ...
      {(() => {
            ...
            >
              ...
              <AnimatePresence mode="wait">
                {refAnterior === undefined ? (
                  ...
                  <motion.div key="ref-data" style={s.refCardCompact} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.25 }}>
        ...
        const serieRef = refAnterior?.series?.find(s => s.numeroSerie === serieActual)
        const refSerieBlock = serieRef ? (
          <motion.div
            key={`ref-serie-${ejIdx}-${serieIdx}`}
            style={s.refSerieCard}
            initial={{ opacity: 0, y: 8 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.25 }}
          >
            <span style={s.refSerieIcon}>🎯</span>
            <div style={{ flex: 1 }}>
              <span style={s.refSerieLabel}>Última vez — serie {serieActual}</span>
